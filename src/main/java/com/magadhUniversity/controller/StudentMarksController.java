...
public class StudentMarksController {
    ...
    // Method to handle "performance" button click and display the graph (optional)
//    @GetMapping("/performance/{studentId}")
//    public String showPerformance(@PathVariable("studentId") Long studentId, Model model) {
//        // Fetch the marks for a specific student if needed
//        List<StudentMarks> studentMarksList = studentMarksService.getPMarksByStudentId(studentId);
//
//        // Pass the student marks list to the Thymeleaf template
//        model.addAttribute("studentMarksList", studentMarksList);
//
//        // Return the view with the graph (this could be a new page or the same list page)
//        return "performance_graph";  // This will render the page with the graph (see step 2 for the template)
//    }

    // Method for handling the "performance" button click. Here we simply return a part of the data.
    @GetMapping("/performance/{studentId}")
    @ResponseBody
    public List<StudentMarks> getStudentPerformance(@PathVariable("studentId") Long studentId) {
        return studentMarksService.getPMarksByStudentId(studentId);
    }
